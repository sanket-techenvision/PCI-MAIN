...
<body>
    <div class="container">
        ...
        <p>HEREBY CONFIRM WITH FULL FINANCIAL RESPONSIBILITY THAT WE ARE READY WILLING AND ABLE TO ISSUE A <span style="color: red;">{{ $letter_type }}</span> REFERENCE TO CONTRACT NO. [CONTRACT NO] DATED [DATE] WITH FACE VALUE <span style="color: red;">{{ $guarantee_amount }}</span> IN FAVOUR OF <span style="color: red;">{{ $beneficiary_first_name }} {{ $beneficiary_last_name }} {{ $beneficiary_address }}, {{ $beneficiary_city }}, {{ $beneficiary_state }}, {{ $beneficiary_country }}</span> IN YOUR GOOD BANK, AND IT SHALL BE TRANSMITTED BY SWIFT MT760.</p>
