...
<body>
    <div class="container">
        <p><span style="color: red;"><b>RWA HC DRAFT</b></span></p>
        ...
        <p>HEREBY CONFIRM WITH FULL FINANCIAL RESPONSIBILITY THAT WE ARE READY WILLING AND ABLE TO ISSUE A <span style="color: red;">PERFORMANCE BOND</span> REFERENCE TO CONTRACT NO. {{$contract_no}} DATED {{$contract_date}} WITH FACE VALUE <span style="color: red;">{{ $currency_code }} {{ $guarantee_amount }} </span> IN FAVOUR OF <span style="color: red;">{{ $beneficiary_company_name }}, {{ $beneficiary_address }}, {{ $beneficiary_city }}, {{ $beneficiary_state }}, {{ $beneficiary_country }}</span> IN YOUR GOOD BANK, AND IT SHALL BE TRANSMITTED BY SWIFT MT760.</p>

        <p>WE FURTHER CONFIRM THAT <span style="color: red;">{{ $bgType }}</span> IS FREE AND CLEARED OF ENCUMBRANCE AND THE CASH IS GOOD, CLEAN AND CLEAR FUNDS OF NON-CRIMINAL ORIGINS.</p>
